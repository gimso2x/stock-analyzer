...
import type { StockQuote, CompanyInfo } from '@/lib/finnhub';
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-lg">
        ...
        <div className="flex items-center gap-2 mt-4 sm:mt-0">
          ...
          <div className={`flex flex-col items-end ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
            <div className="flex items-center gap-1 text-sm font-medium">
              ...
              <span>{isPositive ? '+' : ''}{change?.toFixed(2) || 'N/A'}</span>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-3 bg-slate-50 rounded-lg">
          ...
          <div className="text-sm font-semibold text-slate-900">${quote.o?.toFixed(2) || 'N/A'}</div>
        </div>

        <div className="p-3 bg-slate-50 rounded-lg">
          <div className="text-xs text-slate-600 mb-1">고가</div>
          <div className="text-sm font-semibold text-slate-900">${quote.h?.toFixed(2) || 'N/A'}</div>
        </div>

        <div className="p-3 bg-slate-50 rounded-lg">
          <div className="text-xs text-slate-600 mb-1">저가</div>
          <div className="text-sm font-semibold text-slate-900">${quote.l?.toFixed(2) || 'N/A'}</div>
        </div>

        <div className="p-3 bg-slate-50 rounded-lg">
          <div className="text-xs text-slate-600 mb-1">전일 종가</div>
          <div className="text-sm font-semibold text-slate-900">${quote.pc?.toFixed(2) || 'N/A'}</div>
        </div>
      </div>
    </div>
  );
